...
import { Layers, Truck, Pencil, Trash2, X, Check, CalendarDays } from 'lucide-react'
...
export default function SupplierGroupsPage() {
  ...
  const load = () => {
    setLoading(true)
    fetch('/api/supplier-groups').then(r => r.ok ? r.json() : []).then(setGroups).catch(() => setGroups([])).finally(() => setLoading(false))
  }
  ...
  const addGroup = async () => {
    ...
    await fetch('/api/supplier-groups', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) })
    setShowAdd(false)
    setForm({ ...EMPTY_FORM })
    load()
  }
  ...
  const saveEdit = async () => {
    await fetch('/api/supplier-groups', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(editForm) })
    setEditingId(null)
    load()
  }
  ...
  const deleteItem = async (id: string) => {
    ...
    await fetch('/api/supplier-groups', { method: 'DELETE', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id }) })
    load()
  }
      ...
      <div className="page-header">
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 800, margin: 0 }}>
            ...
            {isAr ? 'مجموعات الموردين' : 'Supplier Groups'}
          </h1>
          <p style={{ fontSize: 14, color: 'var(--text-muted)', margin: '4px 0 0' }}>
            {groups.length} {isAr ? 'مجموعة' : 'groups'}
          </p>
        </div>
        <AddButton onClick={() => setShowAdd(true)} label={isAr ? 'إضافة مجموعة' : 'Add Group'} tooltip={isAr ? 'إضافة مجموعة موردين جديدة' : 'Add new supplier group'} />
      </div>
        ...
        <div className="modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="modal-content" onClick={e => e.stopPropagation()} style={{ maxWidth: 460 }}>
            <div className="modal-header">
              <h2 style={{ fontSize: 16, fontWeight: 700 }}>{isAr ? 'إضافة مجموعة موردين' : 'Add Supplier Group'}</h2>
                  ...
                  <>
                    ...
                    <td>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        ...
                        <span style={{ fontWeight: 600 }}>{g.supplier_count}</span>
                      </div>
                    </td>
                    <td>
