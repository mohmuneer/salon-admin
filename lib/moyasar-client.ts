...
// Moyasar's hosted payment form - card entry happens entirely inside
// Moyasar's own widget, never touching our own state or servers, so this
// deliberately does NOT expose any way to collect raw card numbers ourselves.
const FORM_VERSION = '1.7.3' // double-check this against the live dashboard snippet when setting up the real account
...
export async function startMoyasarCheckout(opts: {
  ...
  Moyasar.init({
    ...
    amount: Math.round(opts.amountSar * 100), // halalas
    currency: 'SAR',
